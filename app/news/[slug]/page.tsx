...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const item = await getBySlug(collections.news, params.slug);
  if (!item) return {};
  const summary = item.excerpt || item.subtitle || `${item.title} — news from Eastern Visayas.`;
...
export default async function NewsDetailPage({ params }: PageProps) {
  ...
  const more = await getNews({ limit: 3 });
  const related = more.filter((n) => n._id !== item._id).slice(0, 3);
    ...
    <>
      ...
      {related.length > 0 && (
        <div className="mx-auto mt-16 max-w-5xl">
          <h2 className="section-title mb-6">More News</h2>
